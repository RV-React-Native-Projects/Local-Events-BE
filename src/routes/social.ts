...
social.get("/following/:followerId/:followingId", async (c) => {
  try {
    ...
    return c.json(
      { success: false, error: "Failed to check following status" },
      500
    );
  }
});
...
social.post(
  ...
  async (c) => {
    try {
      ...
      const hasReviewed = await hasUserReviewedEvent(
        reviewData.userId,
        reviewData.eventId
      );
      if (hasReviewed) {
        return c.json(
          { success: false, error: "Already reviewed this event" },
          400
        );
      }
...
social.delete("/reviews/:reviewId", requireAuth, async (c) => {
  try {
    const reviewId = c.req.param("reviewId");
